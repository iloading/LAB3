...
function jogar() {
  ...
  for (var i = 1; i <= 40; i++) {
    h_random= Math.floor(Math.random()*(innerHeight - hImg + 1))
    w_random= Math.floor(Math.random()*(innerWidth - wImg + 1))
    colisao = false


      //PARTE AVANÇADA é fdd explicar isto por aqui ... se tiverem dúvida perguntem
      for (var c = 1; c <= i; c++) {

        cordY_anterior = parseInt(document.getElementById('lampada'+c).style.top)
        cordX_anterior = parseInt(document.getElementById('lampada'+c).style.left)

        //esqTOP
        if ((w_random+28 > cordX_anterior) && (w_random+28 < cordX_anterior+28) ) {
        if ((h_random+44 > cordY_anterior) && (h_random+44 < cordY_anterior+44) ) {
              i--
              colisao = true
          }
        }
        ...
        //dirTOP
        if ( (w_random >= cordX_anterior) && (w_random <= cordX_anterior+28) ) {
          if ((h_random + 44 >= cordY_anterior) && (h_random+44 <= cordY_anterior+44)) {
            i--
            colisao = true
        }
      ...
      //esqBOT
      if ((w_random+28 > cordX_anterior) && (w_random+28 < cordX_anterior+28) ) {
      if ((h_random > cordY_anterior) && (h_random< cordY_anterior+44) ) {
            i--
            colisao = true
        }
      }
    ...
    if (colisao == false) {
        document.getElementById('lampada'+i).style.top= h_random + 'px';
        document.getElementById('lampada'+i).style.left= w_random + 'px';
    }
